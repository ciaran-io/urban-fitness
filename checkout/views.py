...
def checkout(request):
    ...
    if request.method == 'POST':
            ...
            for item_id, item_data in cart.items():
                try:
                    ...
                    messages.error(request, (
                        "Oops we couldn't find one of your products "
                        "Please get in touch so we can help resolve the problem")
                    )
                    order.delete()
        ...
        if not stripe_public_key:
            messages.warning(
                request, 'Stripe public key is missing. Check if key is set in env variable')
